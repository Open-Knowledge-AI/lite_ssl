...
def oxy_angle_pairwise(
    ...
    """
    Pairwise hyperbolic exterior angle:
        angle(Oxy)
    ...
    x_time = torch.sqrt(1 / curv + torch.sum(x**2, dim=-1))  # [v_l, b]

    y_time = torch.sqrt(1 / curv + torch.sum(y**2, dim=-1))  # [v_g, b]

    # --------------------------------------------------------
    # expand for pairwise computation
    # --------------------------------------------------------

    x_e = x[:, None]  # [v_l, 1, b, d]
    y_e = y[None]  # [1, v_g, b, d]

    xt_e = x_time[:, None]  # [v_l, 1, b]
    yt_e = y_time[None]  # [1, v_g, b]

    # --------------------------------------------------------
    # Lorentz inner product
    # --------------------------------------------------------

    c_xyl = curv * (torch.sum(x_e * y_e, dim=-1) - xt_e * yt_e)  # [v_l, v_g, b]

    # --------------------------------------------------------
    # hyperbolic angle formula
    # --------------------------------------------------------

    acos_numer = yt_e + c_xyl * xt_e

    acos_denom = torch.sqrt(torch.clamp(c_xyl**2 - 1, min=eps))

    x_norm = torch.norm(x, dim=-1)[:, None]  # [v_l, 1, b]
