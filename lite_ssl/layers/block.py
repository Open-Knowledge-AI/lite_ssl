...
def add_residual(x, brange, residual, residual_scale_factor, ls=None):
    if ls is None:
        ...
        x_plus_residual = x_flat.index_add_(
            dim=0, index=brange, source=residual.to(dtype=x.dtype), alpha=residual_scale_factor
        )
    else:
        x_plus_residual = x.index_add_(
            dim=0, source=ls(residual.to(dtype=x.dtype)), index=brange, alpha=residual_scale_factor
        )
...
if __name__ == "__main__":
    ...
    nested_block = NestedTensorBlock(dim=64, num_heads=8, attn_class=MemEffAttention).to(_device)
    nested_x = [
