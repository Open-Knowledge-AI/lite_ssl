...
class ImageNet(ExtendedVisionDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(Path(root) / self.base_folder, transforms, transform, target_transform)
        self._split = ImageNet.Split.TRAIN if train else ImageNet.Split.VAL

        if extra is None:
            extra = self.root  # default to extra being in the root.
