...
class ViTv2(nn.Module):
    ...
    def get_intermediate_layers(
        ...
        class_tokens = [
            out[:, 0] if not norm else self.norm(out[:, : 1 + self.num_register_tokens])[:, 0]
            for out in outputs
        ]
        outputs = [
            (
...
if __name__ == "__main__":
    _model = vitv2_tiny(patch_size=16, num_register_tokens=4).cuda()
    _out = _model(torch.randn(2, 3, 224, 224, device="cuda"), last_self_attention=True)

    print({k: v.shape for k, v in _out.items()})

    _attn = torch.softmax(_out["last_self_attention"], dim=-1)
    print(_attn.sum(-1), _attn.min(dim=-1), _attn.max(dim=-1))
