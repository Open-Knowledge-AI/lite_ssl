...
from lite_ssl.utils import named_apply
...
class ViTv3(nn.Module):
    def __init__(
        ...
        *,
        ...
        pos_embed_rope_dtype: Literal["bf16", "fp16", "fp32"] = "bf16",
        embed_dim: int = 768,
        depth: int = 12,
        num_heads: int = 12,
        ffn_ratio: float = 4.0,
        ...
        norm_layer: Literal["layernorm", "layernormbf16", "rmsnorm"] = "layernormbf16",
        ffn_layer: Literal["mlp", "swiglu"] = "mlp",
        ffn_bias: bool = True,
        proj_bias: bool = True,
        n_storage_tokens: int = 4,
        mask_k_bias: bool = False,
        untie_cls_and_patch_norms: bool = True,
    ...
    def forward_features_list(
        ...
        all_x = x
        output = []
        for idx, (x, hw_tuple, masks) in enumerate(zip(all_x, rope, masks_list)):
            ...
            out = {
                "latent": x_norm_cls_reg[:, 0],
                "patch_latent": x_norm_patch,
                "raw_latent": x[:, 0],
            }
    ...
    def forward_features(
        self, x: Tensor | List[Tensor], masks: Optional[Union[Tensor, List[Tensor]]] = None
    ) -> Union[List[Dict[str, Tensor]], Dict[str, Tensor]]:
        if isinstance(x, torch.Tensor):
            # return self.forward_single(x, masks)
    ...
    def forward(
        ...
        if is_training:
            ...
            return self.head(ret["normed_latent"])


def vitv3_tiny(patch_size=16, **kwargs):
    model = ViTv3(
        patch_size=patch_size,
        embed_dim=192,
        depth=12,
...
def vitv3_small(patch_size=16, **kwargs):
    model = ViTv3(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def vitv3_base(patch_size=16, **kwargs):
    model = ViTv3(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
...
def vitv3_large(patch_size=16, **kwargs):
    model = ViTv3(
        patch_size=patch_size,
        embed_dim=1024,
        depth=24,
...
if __name__ == "__main__":
    _model = vitv3_small(patch_size=16).cuda()

    _out = _model(
        torch.randn(1, 3, 224, 224, device="cuda"),
        torch.randint(0, 2, (1, 14 * 14), dtype=torch.bool, device="cuda"),
        is_training=True,
    )
    print({k: v.shape if isinstance(v, torch.Tensor) else v for (k, v) in _out.items()})
